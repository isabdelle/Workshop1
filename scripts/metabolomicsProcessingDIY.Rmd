...
data <- read_excel("C:/Users/dombeck/Documents/NoMa_NMR_intervention_GEOcodes_Sent Susan.xlsx")
...
sum(is.na(data$`Sample name` == "2_A5")) ##Fill in the missing information at the '...'
...
heatmaply::heatmaply(is.na10(data[,3:100], grid_gap = 1, colors = heat.colors(200), showticklabels = c(T, F), margins = c(80, 10)))
```

## Third programming tasks
```{r}
# Make sure all data is captured as numbers (numeric!)
data <- data.frame(data[,1:2], lapply(data[,3:length(data)],as.numeric))

# Remove rows with too many NAs
data <- data[complete.cases(data[ , 3:length(data)]),]

# Visualize all information in a heatmap
####..... missing step ...#### 
heatmaply::heatmaply(data[,3:100], grid_gap = 1, colors = heat.colors(200), showticklabels = c(T, F), margins = c(80, 10))
#Tip: use the previous line of code for heatmaply, without counting NAs. When in doubt, type ?heatmaply in the console below, to open a help menu for this function and example.
```

## Fourth programming tasks: check data distribution
```{r}
#filter out data if the 'Sample name' column is NA 
data <- subset(data, !is.na(data$Sample.name))
...
data <- subset(data, data$Sample.name != 'missing Gex')
...
#dataLocation <- paste0(getwd(), 'C:/Users/dombeck/Documents/targets.csv')

# Load the targets data
treatmentData <- read.csv('C:/Users/dombeck/Documents/targets.csv', header = TRUE)
...
metabolomicsDataGroups <- merge(treatmentData, data, by="Sample.name")
...
# Visualize all information in a heatmap
heatmaply::heatmaply(metabolomicsDataGroups_transformed[,3:100], grid_gap = 1, colors = heat.colors(200), showticklabels = c(T, F), margins = c(80, 10))
```
